...
static const int DEFAULT_HAS_RECEIVED_PACKET_QUEUE_SIZE = 1024;
static const float PACKETLOSS_TOLERANCE = .02f; // What percentile packetloss we are willing to accept as background noise.
static const float PING_MULTIPLIER_TO_RESEND = 3.5f;
static const RakNetTime MIN_PING_TO_RESEND = 40;
static const RakNetTimeNS TIME_TO_NEW_SAMPLE = 750000; // How many ns to wait before starting a new sample.  This way buffers have time to overflow or relax at the new send rate, if they are indeed going to overflow.

static inline double GetMinimumSendBPS()
{
	return static_cast<double>(SAMPRakNet::GetMinimumSendBitsPerSecond());
}

static inline double GetStartingSendBPS()
{
	return GetMinimumSendBPS() * 2.0;
}
...
ReliabilityLayer::ReliabilityLayer()
	: updateBitStream(DEFAULT_MTU_SIZE) // preallocate the update bitstream so we can avoid a lot of reallocs at runtime
{
...
void ReliabilityLayer::InitializeVariables(void)
{
	...
	currentBandwidth = GetStartingSendBPS();
	...
	lowBandwidth = GetStartingSendBPS();
	...
	if (time >= histogramEndTime)
	{
		...
		if (continuousSend == false)
		{
			if (packetloss > PACKETLOSS_TOLERANCE)
			{
				...
				if (lowBandwidth < GetMinimumSendBPS())
					lowBandwidth = GetMinimumSendBPS();
	...
	*reliableDataSent = false;
	...
	for (i = 0; i < NUMBER_OF_PRIORITIES; i++)
	{
		while (sendPacketSet[i].Size())
		{
			...
			if (unreliableTimeout != 0 && (internalPacket->reliability == UNRELIABLE || internalPacket->reliability == UNRELIABLE_SEQUENCED) && time > internalPacket->creationTime + (RakNetTimeNS)unreliableTimeout)
			{
...
void ReliabilityLayer::DeleteSequencedPacketsInList(unsigned char orderingChannel, DataStructures::List<InternalPacket*>& theList, int splitPacketId)
{
	...
	{
		if ((theList[i]->reliability == RELIABLE_SEQUENCED || theList[i]->reliability == UNRELIABLE_SEQUENCED) && theList[i]->orderingChannel == orderingChannel && (splitPacketId == -1 || theList[i]->splitPacketId != (unsigned int)splitPacketId))
		{
...
bool ReliabilityLayer::IsOlderOrderedPacket(OrderingIndexType newPacketOrderingIndex, OrderingIndexType waitingForPacketOrderingIndex)
{
	...
	else if (newPacketOrderingIndex >= (OrderingIndexType)(waitingForPacketOrderingIndex - ((OrderingIndexType)maxRange / 2 + 1)) || newPacketOrderingIndex < waitingForPacketOrderingIndex)
	{
		return true;
	}
...
void ReliabilityLayer::InsertIntoSplitPacketList(InternalPacket* internalPacket, RakNetTimeNS time)
{
	...
	if (splitMessageProgressInterval && splitPacketChannelList[index]->splitPacketList[0]->splitPacketIndex == 0 && splitPacketChannelList[index]->splitPacketList.Size() != splitPacketChannelList[index]->splitPacketList[0]->splitPacketCount && (splitPacketChannelList[index]->splitPacketList.Size() % splitMessageProgressInterval) == 0)
	{
...
void ReliabilityLayer::DeleteOldUnreliableSplitPackets(RakNetTimeNS time)
{
	...
	{
		if (time > splitPacketChannelList[i]->lastUpdateTime + 10000000 && (splitPacketChannelList[i]->splitPacketList[0]->reliability == UNRELIABLE || splitPacketChannelList[i]->splitPacketList[0]->reliability == UNRELIABLE_SEQUENCED))
		{
			for (j = 0; j < splitPacketChannelList[i]->splitPacketList.Size(); j++)
			{
				delete[] splitPacketChannelList[i]->splitPacketList[j]->data;
